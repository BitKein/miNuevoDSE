...
int main()
{
    ...
    inic_OC1();             // Output Compare (pulso PWM)
    
    inic_Timer2_PWM();      // Timer pulso PWM para el modulo OC1
    ...
    while(1) {
        ...
        if (flag_ADC){
            update_ADC();
            if (!flag_dutyPorUART) // 0 = por potenciometro
                update_duty();
            reiniciar_muestreo();
        }
        
        // Actualizar los duty cycles del servomotor (LCD pagina 3)
        if (flag_OCPWM > 0 && flag_dutyPorUART){
            update_OCPWM();
        }
