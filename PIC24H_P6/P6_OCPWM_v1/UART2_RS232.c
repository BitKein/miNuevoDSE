...
void _ISR_NO_PSV _U2RXInterrupt() {
    ...
    switch(cmd){
        ...
        case 'c':   // c = continuar cronometro (reanudar)
        case 'C':
            T7CONbits.TON = 1;
        break;

        case 'p':   // p = parar cronometro
        case 'P':
            T7CONbits.TON = 0;
        break;
        
        case '+':   // + = incrementar pulso PWM del servomotor por +10 unidades
            flag_OCPWM = FLAGOC_SUMA;
        break;

        case '-':   // - = decrementar pulso PWM del servomotor por -10 unidades
            flag_OCPWM = FLAGOC_RESTA;
        break;
