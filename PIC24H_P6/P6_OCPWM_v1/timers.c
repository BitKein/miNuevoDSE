...
void inic_Timer2_PWM() {  // Inicializa T2
    ...
    PR2 = PR20ms; // Carga valor del periodo a 800k ciclos (12500 * 64)
    T2CONbits.TCKPS = 2; // prescaler 1:64

    IFS0bits.T2IF = 0; // Borra flag interrupción
    IEC0bits.T2IE = 0; // Deshabilita Interrupción Timer 2
    // Con PWM no es necesario utilizar interrupciones
    // Se puede modificar OCxRS en cualquier momento.
